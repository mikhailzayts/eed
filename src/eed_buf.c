...
eed_buf_line_s * eed_buf_line_get_by_idx (eed_buf_s * p_buf, uint32_t idx);
...
bool eed_buf_line_insert (eed_buf_s * p_buf, const char * p_line, uint32_t pos)
{
    ...
    else
    {
        ...
        if (p_buf->len != pos)
        {
            p_dest = eed_buf_line_get_by_idx(p_buf, pos);
        }
...
bool eed_buf_line_remove (eed_buf_s * p_buf, uint32_t pos)
{
    ...
    eed_buf_line_s * p_removed = eed_buf_line_get_by_idx(p_buf, pos);
...
bool eed_buf_line_copy (eed_buf_s * p_buf, uint32_t pos, uint32_t dest)
{
    ...
    eed_buf_line_s * p_copied = eed_buf_line_get_by_idx(p_buf, pos);
...
bool eed_buf_line_move (eed_buf_s * p_buf, uint32_t pos, uint32_t dest)
{
    ...
    eed_buf_line_s * p_moved = eed_buf_line_get_by_idx(p_buf, pos);
    _line_remove(p_buf, p_moved);

    eed_buf_line_s * p_dest = eed_buf_line_get_by_idx(p_buf, dest);
...
bool eed_buf_line_peek (eed_buf_s * p_buf, char ** pp_line, uint32_t pos)
{
    ...
    eed_buf_line_s * p_curr = eed_buf_line_get_by_idx(p_buf, pos);
...
eed_buf_line_s * _mem_alloc (eed_buf_s * p_buf, const char * p_str)
{
    ...
    p_new->p_str      = (char *)eed_mem_alloc(&(p_buf->mem_iface), line_len);
